...
function events_go2Page(id){
	...
	for(var i = events_currentPage*events_pageSize;i<(events_currentPage+1)*events_pageSize&&i<events_totalSize;i++){
		html+="<a class=\"fish\" target=\"_blank\" href=\""+eventsBaseUrl+"/student/content/detail.html?id="+eventsList[i].id+"\" title="+eventsList[i].title+" href=\"#\" title="+eventsList[i].title+"><span class=\"new\">";
		
		var tempTitle;
		if(eventsList[i].title.length>eventsTitleSize){
			tempTitle = eventsList[i].title.substring(0,eventsTitleSize)+"...";
		}else{
