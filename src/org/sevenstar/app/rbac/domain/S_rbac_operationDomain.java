...
@SSDomain(table="s_rbac_operation",condition="sts<>'N'")
@SSId(generateType="seq",seq="seq_app")
@SSCacheModel(selectAll=true)
public class S_rbac_operationDomain extends PersistentObject {
	...
	public void update() {
		this.setSts("Y");
		S_rbac_operationDomain paramDomain = new S_rbac_operationDomain();
		paramDomain.setName(this.getName());
		if (paramDomain.updateExist(this.getId())) {
			throw new ApplicationException("操作名[" + this.getName() + "]已存在");
		}
		super.update();
	}
	...
	public boolean hasOperation(String name) {
		S_rbac_operationDomain param = new S_rbac_operationDomain();
		param.setName(name);
		return param.exist();
	}
